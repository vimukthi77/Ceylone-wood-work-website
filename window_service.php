...
// Fetch window services for the logged-in user
$sql = "SELECT * FROM window_services WHERE user_id = ?";
...
if (isset($_POST['delete'])) {
    ...
    $delete_sql = "DELETE FROM window_services WHERE id = ? AND user_id = ?";
    $delete_stmt = $conn->prepare($delete_sql);
    $delete_stmt->bind_param("ii", $service_id, $user_id);
    $delete_stmt->execute();
    header("Location: window_service.php");
    exit();
}
...
if (isset($_POST['update'])) {
    ...
    $update_sql = "UPDATE window_services SET window_size = ?, window_type = ?, address = ? WHERE id = ? AND user_id = ?";
    $update_stmt = $conn->prepare($update_sql);
    $update_stmt->bind_param("sssii", $window_size, $window_type, $address, $service_id, $user_id);
    $update_stmt->execute();
    header("Location: window_service.php");
    exit();
}
...
if (isset($_GET['generate_pdf'])) {
    class PDF extends FPDF {
        function Header() {
            ...
            $this->Cell(0, 10, 'Window Services Report', 0, 1, 'C');
            $this->Ln(5);
        }
    ...
    $pdf->Cell(20, 7, 'ID', 1);
    $pdf->Cell(40, 7, 'Window Size', 1);
    $pdf->Cell(40, 7, 'Window Type', 1);
    ...
    foreach ($services as $service) {
        ...
        $pdf->Cell(40, 6, $service['window_size'], 1);
        $pdf->Cell(40, 6, $service['window_type'], 1);
        $pdf->Cell(60, 6, $service['address'], 1);
        $pdf->Cell(30, 6, date('Y-m-d', strtotime($service['created_at'])), 1);
        $pdf->Ln();
    }

    $pdf->Output('window_services_report.pdf', 'D');
    exit();
}
?>
...
<body>
    <div class="container">
        <h1 class="mb-4">Your Window Services</h1>
        ...
        <table class="table table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo $service['window_size']; ?></td>
                    <td><?php echo $service['window_type']; ?></td>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="updateModalLabel<?php echo $service['id']; ?>">Update Service</h5>
                            ...
                            <div class="modal-body">
                                <form method="post">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="window_size" name="window_size" value="<?php echo $service['window_size']; ?>" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="window_type" class="form-label">Window Type</label>
                                        <input type="text" class="form-control" id="window_type" name="window_type" value="<?php echo $service['window_type']; ?>" required>
                                    </div>
                                    <div class="mb-3">
